...
# ── Carga de artefactos (cacheados en sesión) ───────────────────────────────
@st.cache_resource
def load_artifacts():
...
def predecir_mes(mes: int, año: int) -> pd.DataFrame:
    ...
    for dia in range(1, n_dias + 1):
        ...
        for tipo in TIPOS:
            ...
            c_l7     = _c(fd - td(7), tc)
            c_mov4s  = (_c(fd-td(7),tc)+_c(fd-td(14),tc)+_c(fd-td(21),tc)+_c(fd-td(28),tc)) / 4.0
...
st.markdown("""
<style>
.kpi-box  { background:#f0f4ff; border-radius:10px; padding:14px 18px; text-align:center; margin-bottom:8px; }
...
tab1, tab2, tab3 = st.tabs(["📋 Modelo", "📈 Métricas", "🔮 Predecir mes"])
...
""")
    ...
    st.caption("*Autónomo*: no necesita datos del día actual para predecir; usa solo historial pasado.")

    st.dataframe(pd.DataFrame([
        ("DIASEM",      "Calendario",  "Día de la semana (0=Lun…6=Dom). Captura el patrón semanal: mayor demanda lun–vie.", "Alta"),
        ("tipo_cod",    "Categórica",  "Tipo de consulta codificado: Adultos=0, Infantil=1, Teleconsulta=2.", "Alta"),
        ("A_FERIADO",   "Binaria",     "1 si la fecha es feriado nacional → VENTAS=0. Evita predecir en días no laborables.", "Alta"),
        ("TENDENCIA",   "Temporal",    "Días desde Abr 2023. Captura el crecimiento sostenido de demanda a largo plazo.", "Media"),
        ("PROM_HIST",   "Histórica",   "Promedio histórico de VENTAS para ese mes y tipo (en MM$). Ancla estacional fuerte.", "Alta"),
        ...
        ("VACACIONES",  "Binaria",     "1 en vacaciones escolares de verano (dic 26–mar 1) o invierno (calendario MINEDUC).", "Media"),
...
<div class="kpi-box">
  ...
  <div class="kpi-lbl">{lbl}<br><span style="font-size:.75rem">{hint}</span></div>
        ...
        if sesgo is not None:
            ...
            st.info(f"**Sesgo medio:** ${sesgo:,.0f}  ({dir_}) — el factor ×1.063 corrige la subestimación sistemática.")
        ...
        fig_imp = px.bar(df_imp, x="Gain", y="Feature", orientation="h",
                         ...
                         labels={"Gain":"Ganancia media (gain)"})
        fig_imp.update_layout(showlegend=False, coloraxis_showscale=False,
                              margin=dict(l=0,r=0,t=10,b=0))
        st.plotly_chart(fig_imp, use_container_width=True)

        # Pred vs Real (si fue guardado)
        if "pred_vs_real" in metrics:
        ...
        ok_r2   = r2   is not None and r2   >= 0.75
        ok_mape = mape is not None and mape <= 18.0
        aprobado = ok_r2 and ok_mape
...
""")
        if aprobado:
            st.success("**✅ APROBADO** — el modelo puede pasar a producción (reentrenar con todo el histórico).")
        else:
            st.error("**❌ RECHAZADO** — se conserva el modelo del mes anterior. Revisar datos y código.")
