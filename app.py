...
""")
    ...
    # Importancia relativa desde gain real del modelo (no hardcodeada)
    _gains = model.get_booster().get_score(importance_type="gain")
    _max_g = max(_gains.values()) if _gains else 1
    def _imp_label(feat):
        g = _gains.get(feat, 0)
        pct = g / _max_g
        if pct >= 0.70: return "Muy alta"
        if pct >= 0.35: return "Alta"
        if pct >= 0.10: return "Media"
        return "Baja"

    _feat_info = [
        ("DIASEM",         "Calendario",  "Día de la semana (1=Lun…7=Dom). Captura el patrón semanal de demanda."),
        ("tipo_cod",       "Categórica",  "Tipo de consulta: Adultos=0, Infantil=1, Teleconsulta=2."),
        ("A_FERIADO",      "Binaria",     "1 si la fecha es feriado nacional → VENTAS=0 ese día."),
        ("TENDENCIA",      "Temporal",    "Días desde Abr 2023. Captura crecimiento de demanda a largo plazo."),
        ("PROM_HIST",      "Histórica",   "Promedio histórico de VENTAS para ese mes y tipo (MM$). Ancla estacional fuerte."),
        ("LAG7",           "Lag ventas",  "VENTAS de hace 7 días exactos (mismo día de semana). Mejor predictor de corto plazo."),
        ("MEDIA_MOV4S",    "Lag ventas",  "Promedio de lags -7/-14/-21/-28 días. Suaviza ruido puntual de LAG7."),
        ("VACACIONES",     "Binaria",     "1 en vacaciones de verano (dic 26–mar 1) o invierno (MINEDUC)."),
        ("CANT_LAG7",      "Lag conteo",  "Cantidad de pacientes atendidos hace 7 días. Volumen sin efecto precio."),
        ("CANT_MOV4S",     "Lag conteo",  "Promedio de cantidad de pacientes en las últimas 4 semanas."),
        ("LAG_RATIO",      "Ratio v6",    "LAG7 / PROM_HIST: posición relativa inmediata vs. histórico del mes."),
        ("MOV4S_RATIO",    "Ratio v6",    "MEDIA_MOV4S / PROM_HIST: tendencia de las últimas 4 semanas normalizada por el histórico."),
        ("CRECIMIENTO_8S", "Ratio v6",    "Promedio de ratios LAG/PROM_HIST en las últimas 8 semanas. Detecta si la clínica está en régimen de crecimiento acelerado."),
    ]
    df_feat = pd.DataFrame(
        [(f, t, d, _imp_label(f)) for f, t, d in _feat_info],
        columns=["Feature", "Tipo", "Descripción", "Importancia relativa"]
    ).sort_values("Importancia relativa",
                  key=lambda s: s.map({"Muy alta":0,"Alta":1,"Media":2,"Baja":3}))
